...
**Correction: an earlier draft of this section incorrectly said "OPS does not have a table literally named `budget_line_item_version`" — that's wrong, the table does exist, and the sentences right after it already said so, which a teammate correctly flagged as self-contradictory.** What's actually true, and what that earlier wording was fumbling toward: `budget_line_item_version` isn't a hand-authored SQLAlchemy model class you'll find by grepping `models/*.py` for `class BudgetLineItemVersion`. It's generated at runtime by `sqlalchemy-continuum` (`models/base.py`: `make_versioned(user_cls=None)`, `BaseModel.__versioned__ = {}`), which builds a shadow `<table>_version` table for every `BaseModel` table, including `budget_line_item_version` — and that table is real, physically present in the database (also visible via `alembic/versions/` migrations that create/alter it). This was confirmed empirically against the real test Postgres DB (via a temporary, since-reverted probe test using the `loaded_db`/`db_with_data` fixtures in `tests/load_remove_budget_lines/`):
...
**Investigated per teammate question: BLI type conversion (e.g. Contract → Grant) deletes and re-creates the row under the same id — does that make an id look simultaneously "deleted" and "live"?** `data_tools/src/update_budget_line_type/utils.py` + `convert_budget_line_item_type` (`common/utils.py:154`) confirm this happens exactly as described: the old subtype row is deleted (`session.delete(old_budget_line_item); session.commit()`), then a brand-new object of the new subtype is inserted with the **same `id`**, copied via `attrs = {c.key: getattr(budget_line_item, c.key) for c in inspect(BudgetLineItem).mapper.column_attrs}` (which includes `id`) — as two separate commits.

Reproduced that exact sequence against the real test DB (temporary, since-reverted probe) and inspected `budget_line_item_version` for the id afterward:
```
tx=5  op=INSERT  end_tx=7      (original creation)
tx=7  op=DELETE  end_tx=8      (deleted during conversion — end_tx is set here, NOT null)
tx=8  op=INSERT  end_tx=None   (reinserted under the new subtype — the only terminal row)
```
Continuum retroactively set the stale DELETE row's `end_transaction_id` once a new row with the same `id` appeared later — it threads version history by the natural `id`, not by object identity, so a delete-then-reinsert cycle collapses into one continuous chain. **There is never more than one terminal (`end_transaction_id IS NULL`) row per id**, so a converted BLI cannot simultaneously look "deleted" and "live" in `budget_line_item_version` — its terminal row's `operation_type` always reflects the current truth.

This script is doubly safe from the scenario regardless: `classify_budget_lines` queries the live `budget_line_item` table first (`get_existing_bli_obe_map`, over *all* input ids) and only passes ids **not** found live into the deleted-check (`get_deleted_bli_ids`). A converted BLI still exists live under its original id, so it's classified as existing before the deleted-check ever runs on it — independent of the continuum-threading guarantee above.
...
**Decision 8 (added after teammate question): BLI type conversion (delete-old-subtype + reinsert-new-subtype under the same id) cannot make an id look simultaneously "deleted" and "live."** No code change from the original plan — this is a confirmation, not a fix, but is called out explicitly because a teammate specifically asked how this common real-world case (converting a BLI from one CIG type to another, which the codebase does via `update_budget_line_type`) is handled.
- Verified empirically (see Background) that continuum threads version history by `id`, so a delete-then-reinsert cycle always collapses to a single terminal row reflecting current truth — never two conflicting terminal rows.
- Also structurally guaranteed by `classify_budget_lines`'s ordering: the live-table check (`get_existing_bli_obe_map`) runs over all ids first, and only its leftovers are checked against `get_deleted_bli_ids`. A converted BLI is found live and never reaches the deleted-check.
- No mitigation needed beyond documenting *why* — but see the new test case below, since "we reasoned it's fine" isn't the same guarantee as "a test exercises the exact production conversion path and confirms it."

## Testing Strategy

### Test Data Needed
`data_tools/test_csv/compare_prod_to_maps_budget_lines_sample.tsv` with 8 rows:
...
4. `Sys_Budget_ID=15006` — created as a `ContractBudgetLineItem`, then converted to a `GrantBudgetLineItem` under the same id, mirroring `convert_budget_line_item_type`'s production sequence exactly: capture attrs from the base row, `session.delete(old_typed_instance); session.commit()`, then `session.add(GrantBudgetLineItem(**attrs)); session.commit()`. Must land in existing-lines (see Decision 8) even though its version history contains a superseded DELETE row.
5. `Sys_Budget_ID=88888` — created then deleted inside the test setup so it's a genuine continuum-tracked deletion: `session.add(bli); session.commit()` (records the continuum INSERT), then `session.delete(bli); session.commit()` (records the continuum DELETE). **Not** via `load_remove_budget_lines.utils.create_models` — that helper is the production delete-path (`session.get` + requires an `agreement` + fires `agreement_history_trigger_func`/`OpsEvent`), which is unrelated scaffolding this test doesn't need just to produce a version-table DELETE row. Plain `session.add`/`session.delete` + commits produces the identical continuum-tracked result with far less setup.
6. `Sys_Budget_ID=99999` — never created; must land in missing-lines.
7. `Sys_Budget_ID` blank — must land in missing-lines, and must not raise/crash the run.
8. `Sys_Budget_ID="ABC"` (non-numeric) — must land in missing-lines, must not raise/crash the run, and must not be logged/counted as a "blank-id" row (see Decision 4a).
...
- [ ] Seed BLI 15000 (`is_obe=False`), left untouched after creation. Seed BLI 15001 (`is_obe=True`), then update one of its fields (e.g. `comments`) so its terminal version row is an `UPDATE`, not an `INSERT`. Seed BLI 15002 with `is_obe` left as `None` (don't pass it to the constructor). Seed BLI 15006 as a `ContractBudgetLineItem`, then convert it to `GrantBudgetLineItem` under the same id via the delete-then-reinsert sequence described in Test Data Needed. Create BLI 88888 via `session.add`/`commit`, then remove it via `session.delete`/`commit` (not `load_remove_budget_lines.utils.create_models` — see Test Data Needed above for why).
- [ ] Run `classify_budget_lines` (or `run(...)` end-to-end against the sample TSV) and assert:
  - Existing-lines output contains exactly rows for 15000, 15001, 15002, **and** 15006, with `OBE` = `"False"` / `"True"` / `""` / (whatever 15006 was seeded with) respectively — 15001 must not be dropped, since its terminal version row also has `end_transaction_id IS NULL` (this is the exact scenario Decision 6's fix targets; asserting only on 15000 wouldn't catch a regression here); 15002's `OBE` must be exactly `""`, not `"None"` or `"False"` (Decision 7); and 15006 must appear despite its version history containing a superseded DELETE row from the type conversion (Decision 8) — assert on `type(session.get(BudgetLineItem, 15006))` being `GrantBudgetLineItem` too, confirming the conversion actually took effect.
  ...
  - Row for 15006 does **not** appear in missing-lines (the regression this test guards against: treating its superseded DELETE row as evidence of deletion).
...
| `classify_budget_lines`'s live-first ordering gets refactored away (e.g. someone "simplifies" it to check `get_deleted_bli_ids` before/instead of `get_existing_bli_obe_map`) | A BLI that was type-converted (deleted old subtype + reinserted new subtype under the same id — happens frequently per teammate report) could get classified as deleted even though it's live | Low today (continuum's own threading also protects against this independently — see Decision 8), but would become a real risk if the ordering guarantee were removed | Integration test (BLI 15006) exercises the exact production type-conversion sequence and asserts the id lands in existing-lines, not missing-lines |
