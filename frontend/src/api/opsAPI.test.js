...
describe("opsAPI - Agreements Pagination", () => {
    ...
    describe("Agreement Name Filter Exact Match", () => {
        it("should add exact_match=true parameter when agreement name filter is provided", async () => {
            let capturedUrl = "";
            server.use(
                http.get("*/api/v1/agreements/", ({ request }) => {
                    capturedUrl = request.url;
            ...
            await storeRef.store.dispatch(
                opsApi.endpoints.getAgreements.initiate({
                    filters: {
                        ...
                        budgetLineStatus: [],
                        portfolio: [],
                        agreementName: [{ display_name: "Test Agreement" }]
                    },
                    onlyMy: false,
                    sortConditions: null,
                    sortDescending: false,
            ...
            expect(capturedUrl).toContain("name=Test%20Agreement");
            expect(capturedUrl).toContain("exact_match=true");
        });

        it("should not add exact_match parameter when no agreement name filter provided", async () => {
            let capturedUrl = "";
            server.use(
                http.get("*/api/v1/agreements/", ({ request }) => {
                    capturedUrl = request.url;
            ...
            expect(capturedUrl).not.toContain("exact_match");
        });

        it("should add exact_match=true with multiple agreement names", async () => {
            let capturedUrl = "";
            server.use(
                http.get("*/api/v1/agreements/", ({ request }) => {
                    capturedUrl = request.url;
            ...
            await storeRef.store.dispatch(
                opsApi.endpoints.getAgreements.initiate({
                    filters: {
                        ...
                        portfolio: [],
                        agreementName: [
                            { display_name: "Agreement 1" },
                            { display_name: "Agreement 2" }
                        ]
                    },
                    onlyMy: false,
                    sortConditions: null,
            ...
            expect(capturedUrl).toContain("name=Agreement%201");
            expect(capturedUrl).toContain("name=Agreement%202");
            expect(capturedUrl).toContain("exact_match=true");
        });
    });
});
