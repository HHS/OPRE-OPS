...
const suite = create((fieldName) => {
    ...
    each(fieldName.budget_line_items, (item, index) => {
        ...
        test(`Budget line item ${index + 1} (${item.line_description})`, "Need by date is required information", () => {
            enforce(item.date_needed).isNotBlank();
        });
        test(`Budget line item ${index + 1} (${item.line_description})`, "Need by date must be in the future", () => {
            const today = new Date().valueOf();
            const dateNeeded = new Date(item.date_needed).valueOf();
            enforce(dateNeeded).greaterThan(today);
        });
        test(`Budget line item ${index + 1} (${item.line_description})`, "CAN is required information", () => {
            enforce(item.can_id).isNotBlank();
        });
        test(`Budget line item ${index + 1} (${item.line_description})`, "Amount must be more than $0", () => {
