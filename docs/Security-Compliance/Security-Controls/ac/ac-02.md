...
c. OPS soles are assigned based on what an indiivdual will be using the system for. OPS users, including those who would function as an account manager, posessing the named role of "User Admin" determine if necessarry conditions are met for a given user's group or role membership based on that user's unique job function. The defined activities of each of the other roles can be found in [AC-05](./ac-05.md) and [AC-03](./ac-03.md)
