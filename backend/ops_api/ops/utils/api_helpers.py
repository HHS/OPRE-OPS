...
def get_change_data(
    ...
    try:
        ...
        data = {}
    change_data = schema.dump(
        schema.load(request_json, unknown=EXCLUDE, partial=partial)
    )
    change_data = {
        key: value
...
def validate_and_prepare_change_data(
    ...
    loaded_data_dict = (
        vars(loaded_data) if not isinstance(loaded_data, dict) else loaded_data
    )
    ...
    filtered_old_data = {
        key: value for key, value in old_data.items() if key in change_data_dict
    }
...
def update_model_instance_data(model_instance: BaseModel, data: dict[str, Any]) -> None:
    for item in data:
        if item in [
            c_attr.key for c_attr in inspect(model_instance).mapper.column_attrs
        ]:
...
def convert_date_strings_to_dates(
    data: dict[str, Any], date_keys: list[str] | None = None
) -> dict[str, Any]:
    if not date_keys:
        date_keys = [
            "period_start",
