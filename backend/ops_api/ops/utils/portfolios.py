...
def get_total_funding(portfolio: Portfolio, fiscal_year: Optional[int] = None) -> TotalFunding:
    ...
    budget_line_items = BudgetLineItem.query.filter(
        BudgetLineItem.can_fiscal_year.has(CANFiscalYear.can.managing_portfolio == portfolio)
    )

    if fiscal_year:
        budget_line_items = budget_line_items.filter(BudgetLineItem.can_fiscal_year.fiscal_year == fiscal_year)
