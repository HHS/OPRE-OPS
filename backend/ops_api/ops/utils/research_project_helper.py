...
class GetResearchProjectFundingSummaryQueryParams:
    portfolio_id: int = desert.field(
        marshmallow.fields.Int(validate=marshmallow.validate.Range(min=1))
    )
    fiscal_year: int = desert.field(
        marshmallow.fields.Int(validate=marshmallow.validate.Range(min=1900))
    )
...
class ResearchProjectHelper:
    ...
    def get_funding_summary(
        ...
        total_funding_stmt = (
            ...
            .join(
                ResearchProject, ResearchProject.id == CAN.managing_research_project_id
            )
        ...
        total_funding_amount = (
            float(total_funding[0][0]) if total_funding != [(None,)] else 0
        )
