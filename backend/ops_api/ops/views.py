...
AGREEMENT_REASON_LIST_API_VIEW_FUNC = AgreementReasonListAPI.as_view(
    "agreement-reason-list"
)

# Agreement History Endpoint - specialized from OpsDBHistory
AGREEMENT_HISTORY_LIST_API_VIEW_FUNC = AgreementHistoryListAPI.as_view(
    "agreement-history-group", OpsDBHistory
)
...
AGREEMENT_AGENCY_ITEM_API_VIEW_FUNC = AgreementAgencyItemAPI.as_view(
    "agreement-agency-item", AgreementAgency
)
AGREEMENT_AGENCY_LIST_API_VIEW_FUNC = AgreementAgencyListAPI.as_view(
    "agreement-agency-group", AgreementAgency
)
...
PORTFOLIO_CANS_API_VIEW_FUNC = PortfolioCansAPI.as_view(
    "portfolio-cans", CANFundingDetails
)
...
PORTFOLIO_URL_ITEM_API_VIEW_FUNC = PortfolioUrlItemAPI.as_view(
    "portfolio-url-item", PortfolioUrl
)
PORTFOLIO_URL_LIST_API_VIEW_FUNC = PortfolioUrlListAPI.as_view(
    "portfolio-url-group", PortfolioUrl
)
...
CAN_HISTORY_LIST_API_VIEW_FUNC = CANHistoryListAPI.as_view(
    "can-history-group", CANHistory
)
...
BUDGET_LINE_ITEMS_LIST_FILTER_OPTION_API_VIEW_FUNC = (
    BudgetLineItemsListFilterOptionAPI.as_view(
...
CAN_FUNDING_SUMMARY_LIST_API_VIEW_FUNC = CANFundingSummaryListAPI.as_view(
    "can-funding-summary-list", CAN
)
...
CAN_FUNDING_BUDGET_ITEM_API_VIEW_FUNC = CANFundingBudgetItemAPI.as_view(
    "can-funding-budget-item", CANFundingBudget
)
CAN_FUNDING_BUDGET_LIST_API_VIEW_FUNC = CANFundingBudgetListAPI.as_view(
    "can-funding-budget-group", CANFundingBudget
)

# FUNDING DETAILS ENDPOINTS
CAN_FUNDING_DETAILS_ITEM_API_VIEW_FUNC = CANFundingDetailsItemAPI.as_view(
    "can-funding-details-item", CANFundingDetails
)
...
ADMINISTRATIVE_AND_SUPPORT_PROJECT_ITEM_API_VIEW_FUNC = (
    AdministrativeAndSupportProjectItemAPI.as_view(
...
ADMINISTRATIVE_AND_SUPPORT_PROJECT_LIST_API_VIEW_FUNC = (
    AdministrativeAndSupportProjectListAPI.as_view(
        "administrative-and-support-projects-group", AdministrativeAndSupportProject
    )
)

# RESEARCH TYPE ENDPOINTS
RESEARCH_TYPE_LIST_API_VIEW_FUNC = ResearchTypeListAPI.as_view(
    "research-type-group", ResearchType
)
...
NOTIFICATIONS_LIST_API_VIEW_FUNC = NotificationListAPI.as_view(
    "notifications-group", Notification
)

# ServicesComponent ENDPOINTS
SERVICES_COMPONENT_ITEM_API_VIEW_FUNC = ServicesComponentItemAPI.as_view(
    "services-component-item", ServicesComponent
)
SERVICES_COMPONENT_LIST_API_VIEW_FUNC = ServicesComponentListAPI.as_view(
    "services-component-group", ServicesComponent
)
...
PROCUREMENT_STEP_LIST_API_VIEW_FUNC = ProcurementStepListAPI.as_view(
    "procurement-step-group", ProcurementStep
)

# Procurement: AcquisitionPlanning ENDPOINT
PROCUREMENT_ACQUISITION_PLANNING_ITEM_API_VIEW_FUNC = (
    AcquisitionPlanningItemAPI.as_view(
...
PROCUREMENT_SOLICITATION_ITEM_API_VIEW_FUNC = SolicitationItemAPI.as_view(
    "procurement-solicitation-item", Solicitation
)

# Procurement: Evaluation ENDPOINT
PROCUREMENT_EVALUATION_ITEM_API_VIEW_FUNC = EvaluationItemAPI.as_view(
    "procurement-evaluation-item", Evaluation
)

# Procurement: PreAward ENDPOINT
PROCUREMENT_PRE_AWARD_ITEM_API_VIEW_FUNC = EvaluationItemAPI.as_view(
    "procurement-pre-award-item", PreAward
)
...
CHANGE_REQUEST_LIST_API_VIEW_FUNC = ChangeRequestListAPI.as_view(
    "change-requests-list", ChangeRequest
)
