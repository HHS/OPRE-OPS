...
from ops_api.ops.document.document_repository_factory import (
...
def repository(document_repository_factory):
    document_repository_factory.register_provider(
        "mockfakeauth", FakeDocumentRepository()
    )
...
class TestDocumentRepositoryFactory(BaseDocumentTestCase):
    def setUp(self):
        ...
        self.mock_repository_factory.register_provider(
            self.provider, FakeDocumentRepository()
        )
        self.mock_repository = self.mock_repository_factory.create_repository(
            self.provider
        )
    ...
    def test_register_provider(self):
        ...
        self.mock_repository_factory.register_provider(
            self.provider, mock_repository_class
        )
        assert (
            self.mock_repository_factory._registered_providers[self.provider]
    ...
    def test_add_document(self, mock_insert):
        ...
        assert (
            ...
            == f"FakeDocumentRepository/{created_document['uuid']}"
        )

    def test_add_existing_document_raises_error(
        self,
    ):
        self.mock_repository.documents = {
            self.mock_document_data["document_id"]: self.mock_document_data
        }
    ...
    def test_update_document(self):
        ...
        self.mock_repository.update_document(
            ...
            document_content={
                "document_name": mock_update,
                "document_type": "PDF",
                "agreement_id": 123,
            },
        )

        updated_document = self.mock_repository.get_document(
            self.mock_document_data["document_id"]
        )
    ...
    def test_delete_document(self):
        ...
        deleted_document = self.mock_repository.get_document(
            self.mock_document_data["document_id"]
        )
    ...
    def test_update_status_nonexistent_document(self, mock_process_status_update):
        mock_process_status_update.side_effect = DocumentNotFoundError(
            "Document not found"
        )
