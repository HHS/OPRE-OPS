...
def test_administrative_and_support_projects_search(auth_client, loaded_db):
    ...
    response = auth_client.get(
        url_for("api.projects-group", project_search=["SP1", "SP2"], project_type=[ProjectType.ADMINISTRATIVE_AND_SUPPORT.name])
    )
    ...
    response = auth_client.get(
        url_for("api.projects-group", project_search=["SP2"], project_type=[ProjectType.ADMINISTRATIVE_AND_SUPPORT.name])
    )
