...
def test_portfolio_url_lookup(loaded_db):
    ...
    assert (
        pUrl.url
        == "https://acf.gov/opre/topic/overview/abuse-neglect-adoption-foster-care"
    )
...
def test_portfolio_url_post(budget_team_auth_client, mocker, loaded_db):
    ...
    mock_output_data = PortfolioUrl(
        portfolio_id=10, url="https://acf.gov/opre/topic/overview/test"
    )

    mocker_create_portfolio_url = mocker.patch(
        "ops_api.ops.services.portfolio_url.PortfolioUrlService.create"
    )
    ...
    assert (
        context_manager.metadata["new_portfolio_url"]["portfolio_id"]
        == mock_output_data.portfolio_id
    )
...
def test_portfolio_url_patch(budget_team_auth_client, mocker):
    ...
    old_portfolio_url = PortfolioUrl(
        portfolio_id=10, url="https://acf.gov/opre/topic/overview/test"
    )
    portfolio_url = PortfolioUrl(
        portfolio_id=10, url="https://acf.gov/opre/topic/overview/newtest"
    )

    mocker_update_portfolio_url = mocker.patch(
        "ops_api.ops.services.portfolio_url.PortfolioUrlService.update"
    )
    mocker_get_portfolio_url = mocker.patch(
        "ops_api.ops.services.portfolio_url.PortfolioUrlService.get"
    )
    ...
    response = budget_team_auth_client.patch(
        f"/api/v1/portfolios-url/{test_portfolio_url_id}", json=update_data
    )
    ...
    assert response.status_code == 200
    mocker_update_portfolio_url.assert_called_once_with(
        update_data, test_portfolio_url_id
    )
    assert response.json["url"] == portfolio_url.url


@pytest.mark.usefixtures("app_ctx")
def test_portfolio_url_patch_404(budget_team_auth_client):
...
def test_service_patch_portfolio_url(loaded_db):
    ...
    updated_portfolio_url_ = portfolio_url_service.update(
        update_data, new_portfolio_url.id
    )
...
def test_portfolio_url_put_404(budget_team_auth_client):
    ...
    response = budget_team_auth_client.put(
        f"/api/v1/portfolios-url/{test_funding_received_id}", json=input_data
    )
...
@pytest.mark.usefixtures("app_ctx")
def test_portfolio_url_delete(auth_client, mocker, test_budget_team_user):
...
def test_portoflio_url_delete_404(budget_team_auth_client, mocker):
    ...
    response = budget_team_auth_client.delete(
        f"/api/v1/portfolios-url{test_portfolio_url_id}"
    )
