...
def test_agreement_total_sort_with_none_amount():
    ...
    bli_1 = MagicMock(status=BudgetLineItemStatus.PLANNED, amount=Decimal("100.00"), fees=Decimal("5.00"))
    bli_2 = MagicMock(status=BudgetLineItemStatus.IN_EXECUTION, amount=None, fees=Decimal("0.00"))  # None amount
    bli_3 = MagicMock(status=BudgetLineItemStatus.OBLIGATED, amount=Decimal("300.00"), fees=Decimal("15.00"))
    bli_4 = MagicMock(
        status=BudgetLineItemStatus.DRAFT, amount=Decimal("400.00"), fees=Decimal("20.00")
    )  # Should be excluded
