...
def test_update_agreement_agreement_history_trigger(loaded_db):
    ...
    assert (
        ...
        == "Changes made to the OPRE budget spreadsheet changed the agreement title from Interoperability Initiatives to Interoperability Initiatives Test."
    )
    ...
    assert (
        ...
        == "Changes made to the OPRE budget spreadsheet changed the Reason for Agreement from New Requirement to Recompete."
    )
...
def test_update_bli_status_change_history_trigger(loaded_db):
    ...
    assert (
        new_agreement_history_item.history_message
        == "System Owner requested a status change on BL 15007 from Planned to Executing and it's currently In Review for approval."
    )
...
def test_update_bli_properties_change_history_trigger(loaded_db):
    ...
    assert (
        new_agreement_history_item.history_message
        == "System Owner requested a budget change on BL 15007 from Obligate By on 06/13/2043 to 07/13/2043 and it's currently In Review for approval."
    )
...
def test_agreement_history_change_request_approve_deny(loaded_db):
    ...
    assert (
        new_agreement_history_item.history_message
        == "Director Derrek approved the status change on BL 15007 from Planned to Executing as requested by System Owner."
    )
    ...
    assert (
        can_agreement_history_item.history_message
        == "Director Derrek approved the budget change on BL 15007 from Obligate By on 06/13/2043 to 07/13/2043 as requested by System Owner."
    )
...
def test_proc_shop_change_requests(loaded_db):
    ...
    assert (
        proc_shop_change_request.history_message
        == "Director Derrek approved the change on the Procurement Shop from GCS to IBC as requested by System Owner. This changes the fee rate from 0% to 4.80% and the fee total from $0.00 to $48,000.00."
    )
    ...
    assert (
        proc_shop_change_request.history_message
        == "Director Derrek declined the change on the Procurement Shop from GCS to IBC as requested by System Owner."
    )
...
def test_agreement_history_services_components(loaded_db):
    ...
    assert (
        ...
        == "Changes made to the OPRE budget spreadsheet changed the Period of Performance - End date for Services Component SC22 from 06/30/2024 to 07/15/2054."
    )
