...
def test_agreement_history_direct_draft_to_planned_status_change(loaded_db, app_ctx):
    ...
    assert history_item.history_message == f"{test_user_name} changed the status for BL 16043 from Draft to Planned."
...
def test_agreement_history_solicitation_step(loaded_db, app_ctx):
    # clean up existing AgreementHistory entries before this test
    loaded_db.query(AgreementHistory).delete()
    loaded_db.flush()

    next_agreement_history_ops_event = loaded_db.get(OpsEvent, 71)
    agreement_history_trigger(next_agreement_history_ops_event, loaded_db)
    ...
    assert new_agreement_history_item.history_type == AgreementHistoryType.PROCUREMENT_TRACKER_STEP_UPDATED
    assert new_agreement_history_item.history_title == "Solicitation Completed"
    assert (
        new_agreement_history_item.history_message
        == "User Demo completed step 3 of the Procurement Tracker. The solicitation has been closed to vendors and evaluations can start."
    )
