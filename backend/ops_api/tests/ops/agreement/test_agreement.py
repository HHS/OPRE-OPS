...
@pytest.mark.usefixtures("app_ctx")
def test_agreement_create_contract_agreement(
    loaded_db, contract_agreement_for_create_test
):
    stmt = select(Agreement).where(
        Agreement.id == contract_agreement_for_create_test.id
    )
    agreement = loaded_db.scalar(stmt)
...
@pytest.mark.usefixtures("app_ctx")
def test_agreements_post_duplicate_name_case_insensitive(
    auth_client, loaded_db, test_contract
):
