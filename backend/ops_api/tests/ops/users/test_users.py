...
@pytest.mark.usefixtures("app_ctx")
def test_get_users_by_id_with_auth(client, loaded_db):
...
@pytest.mark.usefixtures("app_ctx")
def test_get_someone_else_user_with_auth(client, loaded_db):
    user = loaded_db.session.get(User, "00000000-0000-1111-a111-000000000004")
    access_token = create_access_token(identity=user)
...
@pytest.mark.usefixtures("app_ctx")
def test_get_all_users_with_auth(client, loaded_db):
    user = loaded_db.session.get(User, "00000000-0000-1111-a111-000000000004")
    access_token = create_access_token(identity=user)

    response = client.get("/api/v1/users/", headers={"Authorization": f"Bearer {str(access_token)}"})
    assert response.status_code == 200
