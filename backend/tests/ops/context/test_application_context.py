...
def test_application_context_register():
    ...
    assert ApplicationContext.get_context().get_name(TestContext) == "TestContext"


def test_test_context_get_name():
    assert TestContext.get_name(TestContext) == "TestContext"


def test_context_auth_library():
    assert TestContext.auth_library(TestContext) is not None
