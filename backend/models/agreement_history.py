...
class AgreementHistory(BaseModel):
    ...
    ops_event_id_record: Mapped[int] = mapped_column(Integer, nullable=True)
...
def format_fiscal_year(timestamp) -> int:
    ...
    if isinstance(timestamp, str):
        parsed_timestamp = datetime.fromisoformat(timestamp[:-1]).astimezone(
            timezone.utc
        )
...
def agreement_history_trigger_func(
    event: OpsEvent, session: Session, system_user: User, dry_run: bool = False
):
    """
    ...
    if event_user is None:
        ...
        logger.error(
            f"Event user for event {event.id} is None. Using placeholder user."
        )
    ...
    match event.event_type:
        ...
        case OpsEventType.UPDATE_BLI:
            history_events.extend(
                create_bli_update_history_events(
                    event, event_user, updated_by_system_user, session, system_user
                )
            )
        case OpsEventType.DELETE_BLI:
            history_events.append(
                AgreementHistory(
                    ...
                    agreement_id_record=event.event_details["deleted_bli"][
                        "agreement_id"
                    ],
                    budget_line_id=None,
        ...
        case OpsEventType.UPDATE_PROCUREMENT_SHOP:
            history_events.extend(
                create_proc_shop_fee_history_events(
                    event, session, system_user, event_user
                )
            )
        ...
        case OpsEventType.UPDATE_AGREEMENT:
            ...
            if event.event_details["agreement_updates"].get("team_member_changes"):
                ...
                team_member_changes = event.event_details["agreement_updates"][
                    "team_member_changes"
                ]
                for item in team_member_changes.get("user_ids_added", []):
                    ...
                    history_events.append(
                        AgreementHistory(
                            agreement_id=event.event_details["agreement_updates"][
                                "owner_id"
                            ],
                            agreement_id_record=event.event_details[
                                "agreement_updates"
                            ]["owner_id"],
                            ops_event_id=event.id,
                            ops_event_id_record=event.id,
                            ...
                            timestamp=event.created_on.strftime(
                                "%Y-%m-%dT%H:%M:%S.%fZ"
                            ),
        ...
        case OpsEventType.CREATE_CHANGE_REQUEST:
            ...
            history_event = create_change_request_history_event(
                change_request, event, session, new_change_request=True
            )
            if history_event:
        ...
        case OpsEventType.UPDATE_SERVICES_COMPONENT:
            history_events.extend(
                create_services_component_history_event(
                    event, event_user, updated_by_system_user
                )
            )
        case OpsEventType.DELETE_SERVICES_COMPONENT:
            history_events.append(
                AgreementHistory(
                    agreement_id=event.event_details["service_component"][
                        "agreement_id"
                    ],
                    agreement_id_record=event.event_details["service_component"][
                        "agreement_id"
                    ],
                    ops_event_id=event.id,
                    ops_event_id_record=event.id,
...
def create_change_request_history_event(
    ...
    if change_request:
        ...
        if reviewer_user is None and change_request["status"] in [
            ...
            logger.error(
                f"Reviewer user for change request {change_request['id']} is None. Using placeholder user."
            )
        change_request_status = (
            "approved" if change_request["status"] == "APPROVED" else "declined"
        )
        if property_changed == "status":
            ...
            old_can = session.get(
                CAN, change_request["requested_change_diff"][property_changed]["old"]
            )
            new_can = session.get(
                CAN, change_request["requested_change_diff"][property_changed]["new"]
            )
            if old_can:
            ...
            old_date_value = change_request["requested_change_diff"][property_changed][
                "old"
            ]
            new_date_value = change_request["requested_change_diff"][property_changed][
                "new"
            ]
            if old_date_value is None or old_date_value == "":
                old_date = "None"
            else:
                old_date = datetime.strftime(
                    datetime.strptime(old_date_value, "%Y-%m-%d"), "%m/%d/%Y"
                )
            if new_date_value is None or new_date_value == "":
                new_date = "None"
            else:
                new_date = datetime.strftime(
                    datetime.strptime(new_date_value, "%Y-%m-%d"), "%m/%d/%Y"
                )
            ...
            old_proc_shop_fee_percentage = (
                old_proc_shop.fee_percentage
                if old_proc_shop and old_proc_shop.fee_percentage
                else 0
            )
            new_proc_shop_fee_percentage = (
                new_proc_shop.fee_percentage
                if new_proc_shop and new_proc_shop.fee_percentage
                else 0
            )
            if old_proc_shop:
    ...
    if property_name in simple_property_names:
        ...
        match property_name:
            ...
            case "requesting_agency_id" | "servicing_agency_id":
                ...
                agency_type_string = (
                    "Requesting Agency"
                    if property_name == "requesting_agency_id"
                    else "Servicing Agency"
                )
                return AgreementHistory(
                    agreement_id=get_agreement_id_from_agreement(agreement),
                    agreement_id_record=agreement_id,
            ...
            case _:
                logger.info(
                    f"{property_name} changed by {updated_by_user.full_name} from {old_value} to {new_value}"
                )
                return None
...
def create_proc_shop_fee_history_events(
    event: OpsEvent, session: Session, system_user: User, event_user: User
):
    from models import ProcurementShop
    ...
    for key in fee_change_dict:
        if key == "fee":
            old_value = (
                fee_change_dict[key]["old_value"]
                if fee_change_dict[key]["old_value"] != 0.0
                else 0
            )
            new_value = (
                fee_change_dict[key]["new_value"]
                if fee_change_dict[key]["new_value"] != 0.0
                else 0
            )
            ...
            matching_agreements = (
                ...
                .where(Agreement.awarding_entity_id == proc_shop_id)
                .all()
            )
...
def create_bli_update_history_events(
    ...
    for key in bli_change_dict:
        ...
        if key == "can_id":
            ...
            if old_value:
                old_date = datetime.strftime(
                    datetime.strptime(old_value, "%Y-%m-%d"), "%m/%d/%Y"
                )
            if new_value:
                new_date = datetime.strftime(
                    datetime.strptime(new_value, "%Y-%m-%d"), "%m/%d/%Y"
                )
            if updated_by_system_user:
...
def create_services_component_history_event(
    event: OpsEvent, event_user: User, system_user_created_event: bool
):
    ...
    for key in sc_change_dict:
        ...
        history_events.append(
            AgreementHistory(
                agreement_id=event.event_details["services_component_updates"][
                    "owner_id"
                ],
                agreement_id_record=event.event_details["services_component_updates"][
                    "owner_id"
                ],
                ops_event_id=event.id,
                ops_event_id_record=event.id,
...
def add_history_events(events: List[AgreementHistory], session: Session) -> None:
    ...
    for event in events:
        ...
        for item in session.new:
            if (
                isinstance(item, AgreementHistory)
                and item.agreement_id_record == event.agreement_id_record
            ):
...
def is_timespan_within_one_minute(
    datetime_to_check: str, reference_datetime: str
) -> bool:
    """Check if two ISO format timespan strings are within one minute of each other."""
    try:
        datetime_to_check_dt = datetime.fromisoformat(
            datetime_to_check.replace("Z", "+00:00")
        )
        reference_datetime_dt = datetime.fromisoformat(
            reference_datetime.replace("Z", "+00:00")
        )
