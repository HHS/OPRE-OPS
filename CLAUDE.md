...
The CI pipeline automatically detects flaky E2E tests by analyzing Cypress retry patterns. When tests fail initially but pass on retry, they're flagged in the GitHub Actions job summary.

**How it works**:
- Cypress is configured with 3 retry attempts in CI (`cypress.config.ci.js`)
- A detection script (`.github/scripts/detect-flaky-tests.sh`) parses Cypress output logs
- Flaky tests are reported in the job summary with recommended actions
